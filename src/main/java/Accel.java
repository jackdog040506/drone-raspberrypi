...
public class Accel {
   ...
   public void readAxis() throws IOException{
      ...
      byte[] buffx = new byte[2];
      byte[] buffy = new byte[2];
      byte[] buffz = new byte[2];
      device.read(ADXL345_REG_DATAX0,buffx,0,2);
      device.read(ADXL345_REG_DATAY0,buffy,0,2);
      device.read(ADXL345_REG_DATAZ0,buffz,0,2);
      int responsex = (asInt(buffx[0]) | ((int)buffx[1])<< 8);
      int responsey = (asInt(buffy[0]) | ((int)buffy[1])<< 8);
      int responsez = (asInt(buffz[0]) | ((int)buffz[1])<< 8);
      double x = (double)responsex * ADXL345_MG2G_MULTIPLIER * SENSOR_GRAVITY_STANDARD;
      double y = (double)responsey * ADXL345_MG2G_MULTIPLIER * SENSOR_GRAVITY_STANDARD;
      double z = (double)responsez * ADXL345_MG2G_MULTIPLIER * SENSOR_GRAVITY_STANDARD;
